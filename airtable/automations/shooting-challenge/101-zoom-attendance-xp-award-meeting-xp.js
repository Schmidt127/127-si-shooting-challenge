...
Important Tables:
Zoom Meetings, Zoom Attendance, Enrollments, XP Reward Rules, XP Events,
Weekly Athlete Summary

Important Fields:
Create XP Events, Attendees, Week, XP Award Status, Recording Pending Reconcile Tokens
 ...
 * - Awards approved recording half-XP without writing Attendees.
 ...
 * - 2026-08-21 v6.7: Roster-expand while Meeting Status is Scheduled/In Progress
 *   no longer deactivates owned events or freezes the reconciled signature without
 *   awards. Positive award still requires Completed. Cancelled / roster-remove /
 *   inactive Enrollment / School Year mismatch still deactivate.
 *
 * IMPORTANT DESIGN RULES
 * - Uses shared XP Reward Rules:
 *      ZOOM_ATTEND_BASE
 *      ZOOM_ATTEND_BONUS_2
 *      ZOOM_ATTEND_BONUS_3
 *      ZOOM_RECORDING (optional; else floor(live/2))
 *
 ...
 *
 * - Recording Source Key:
 *      ZOOM_RECORDING_CREDIT|{Enrollment RID}|{Zoom Meeting RID}
 * - Live Source Key remains:
 *      ZOOM_ATTEND_BASE|{Zoom Meeting Key}|{Enrollment RID}
 * - Recording credit never writes Zoom Meetings.Attendees.
 * - Recording credit does not count toward Perfect Week (057 reads live Attendees).
 * - Do not acknowledge a REC_PENDING-only change merely because Create XP Events
 *   is unchecked.
 *
 * FOLDER
 ...
 * - Zoom Meetings.XP Award Status = Awarded (live award path)
 * - Zoom Meetings.Create XP Events = unchecked (live award path)
 * - Zoom Meetings.XP Awarded At, if field exists
 * - Zoom Meetings.XP Award Error, if field exists
 * - XP Events for ZOOM_RECORDING_CREDIT when recording is approved and Completed
 *
 * RECONCILIATION
 ...
 * - No Automation 121 / separate recording slot required (capacity full).
 ************************************************************/
...
const CONFIG = {
  ...
  zoom: {
    ...
    zoomAttendance: "Zoom Attendance",
    recordingPendingTokens: "Recording Pending Reconcile Tokens",
  },
  ...
  zoomAttendance: {
    ...
    pendingToken: "Recording Pending Reconcile Token",
  },
  ...
  xpLabels: {
    ...
    recordingSourceFallback: "Zoom Meeting Recording Quiz",
...
function parseRecordingPendingTokens(raw) {
  const text = String(raw || "").trim();
  if (!text) return [];
  const tokens = [];
  const seen = new Set();
  const re = /(rec[A-Za-z0-9]{14})\|(rec[A-Za-z0-9]{14})\|REC/g;
  let match;
  while ((match = re.exec(text)) !== null) {
    const enrollmentId = match[1];
    const zoomMeetingId = match[2];
    const key = `${enrollmentId}|${zoomMeetingId}|REC`;
    if (seen.has(key)) continue;
    seen.add(key);
    tokens.push({ enrollmentId, zoomMeetingId, token: key });
  }
  return tokens;
}

function buildRecordingSourceKey(enrollmentId, zoomMeetingId) {
  return `${CONFIG.sourceKeys.recordingPrefix}|${enrollmentId}|${zoomMeetingId}`;
}

function resolveRecordingXpAmount(ruleQueryRecords, configPercent) {
  const liveRules = ruleQueryRecords.filter(record =>
    normalizeRuleKey(getText(record, rulesTable, CONFIG.xpRewardRules.ruleKey)) ===
    normalizeRuleKey(CONFIG.ruleKeys.base) &&
    (!fieldExists(rulesTable, CONFIG.xpRewardRules.active) ||
      getBooleanish(record, rulesTable, CONFIG.xpRewardRules.active))
  );
  if (liveRules.length !== 1) {
    throw new Error(
      liveRules.length === 0
        ? `Base Zoom XP rule not found. Expected active XP Reward Rule with Rule Key: ${CONFIG.ruleKeys.base}`
        : `Multiple active Base Zoom XP rules found for Rule Key "${CONFIG.ruleKeys.base}".`
    );
  }
  const liveAmount = getNumber(liveRules[0], rulesTable, CONFIG.xpRewardRules.xpAmount, null);
  if (!Number.isFinite(liveAmount) || liveAmount <= 0) {
    throw new Error(`Base Zoom XP rule "${CONFIG.ruleKeys.base}" has blank, zero, or invalid XP Amount.`);
  }

  const recordingRules = ruleQueryRecords.filter(record =>
    normalizeRuleKey(getText(record, rulesTable, CONFIG.xpRewardRules.ruleKey)) ===
    normalizeRuleKey(CONFIG.ruleKeys.recording) &&
    (!fieldExists(rulesTable, CONFIG.xpRewardRules.active) ||
      getBooleanish(record, rulesTable, CONFIG.xpRewardRules.active))
  );
  if (recordingRules.length > 1) {
    throw new Error(`Multiple active recording XP rules found for Rule Key "${CONFIG.ruleKeys.recording}".`);
  }

  let xpAmount;
  let xpSourceLabel = CONFIG.xpLabels.recordingSourceFallback;
  if (recordingRules.length === 1) {
    const recordingAmount = getNumber(
      recordingRules[0],
      rulesTable,
      CONFIG.xpRewardRules.xpAmount,
      null
    );
    if (!Number.isFinite(recordingAmount) || recordingAmount < 0) {
      throw new Error(`Recording XP rule "${CONFIG.ruleKeys.recording}" has invalid XP Amount.`);
    }
    xpAmount = Math.floor(recordingAmount);
    xpSourceLabel =
      getText(recordingRules[0], rulesTable, CONFIG.xpRewardRules.xpSourceLabel) ||
      CONFIG.xpLabels.recordingSourceFallback;
  } else if (
    configPercent !== undefined &&
    configPercent !== null &&
    String(configPercent).trim() !== ""
  ) {
    const pct = Math.trunc(Number(configPercent));
    if (pct >= 0 && pct <= 100) {
      xpAmount = Math.floor((liveAmount * pct) / 100);
    } else {
      xpAmount = Math.floor(liveAmount / 2);
    }
  } else {
    xpAmount = Math.floor(liveAmount / 2);
  }

  return { xpAmount, xpSourceLabel, liveAmount };
}

function hasActiveLiveCreditForPair(xpRecords, enrollmentId, zoomMeetingId, meetingKey) {
  const liveBaseKey = buildBaseSourceKey(meetingKey, enrollmentId);
  const liveCanonicalKey = `${CONFIG.sourceKeys.liveCanonicalPrefix}|${zoomMeetingId}|${enrollmentId}`;
  for (const record of xpRecords) {
    if (!getBooleanish(record, xpEventsTable, CONFIG.xpEvents.active)) continue;
    const sourceKey = getText(record, xpEventsTable, CONFIG.xpEvents.sourceKey);
    if (
      normalizeKey(sourceKey) === normalizeKey(liveBaseKey) ||
      normalizeKey(sourceKey) === normalizeKey(liveCanonicalKey)
    ) {
      return true;
    }
  }
...
async function processRecordingCreditsForMeeting({
  recordId,
  meetingKey,
  meetingStatus,
  meetingDateKey,
  ...
  const result = {
    ...
    warnings: [],
    formulaSignatureMustChange: false,
    actionOut: "",
  };
  ...
  if (normalizeText(meetingStatus) !== normalizeText(CONFIG.statuses.completed)) {
    result.awaitingCompleted = true;
    result.actionOut = "reconciled_awaiting_completed_for_recording";
    return result;
  }
  ...
  const { xpAmount, xpSourceLabel } = resolveRecordingXpAmount(ruleQueryRecords, configPercent);

  let attendanceByEnrollment = new Map();
  if (zoomAttendanceTable && fieldExists(zoomTable, CONFIG.zoom.zoomAttendance)) {
    const attendanceQuery = await zoomAttendanceTable.selectRecordsAsync({
      fields: buildFieldsToLoad(zoomAttendanceTable, [
        CONFIG.zoomAttendance.enrollment,
        CONFIG.zoomAttendance.zoomMeeting,
        CONFIG.zoomAttendance.attendanceMethod,
        CONFIG.zoomAttendance.satisfactory,
        CONFIG.zoomAttendance.conflict,
        CONFIG.zoomAttendance.pendingToken,
      ]),
    });
    for (const row of attendanceQuery.records) {
      const meetingIds = getLinkedRecordIds(row, zoomAttendanceTable, CONFIG.zoomAttendance.zoomMeeting);
      if (!meetingIds.includes(recordId)) continue;
      const enrollmentIds = getLinkedRecordIds(row, zoomAttendanceTable, CONFIG.zoomAttendance.enrollment);
      if (enrollmentIds.length !== 1) continue;
      attendanceByEnrollment.set(enrollmentIds[0], row);
    }
  }

  for (const token of pendingTokens) {
    if (token.zoomMeetingId !== recordId) {
      result.skipped += 1;
      continue;
    }

    const enrollmentId = token.enrollmentId;
    const sourceKey = buildRecordingSourceKey(enrollmentId, recordId);
    const attendance = attendanceByEnrollment.get(enrollmentId) || null;

    const quizApproved = attendance
      ? getBooleanish(attendance, zoomAttendanceTable, CONFIG.zoomAttendance.satisfactory)
      : true;
    const conflict = attendance
      ? getNumericValue(attendance, zoomAttendanceTable, CONFIG.zoomAttendance.conflict)
      : 0;
    const method = attendance
      ? getText(attendance, zoomAttendanceTable, CONFIG.zoomAttendance.attendanceMethod)
      : CONFIG.recording.methodValue;

    if (
      attendance &&
      normalizeText(method) &&
      normalizeText(method) !== normalizeText(CONFIG.recording.methodValue)
    ) {
      result.skipped += 1;
      continue;
    }
    ...
    const existingRecording = findSourceKeyEvents(xpQueryRecords, sourceKey).filter(record =>
      getBooleanish(record, xpEventsTable, CONFIG.xpEvents.active)
    );
    ...
    if (hasActiveLiveCreditForPair(xpQueryRecords, enrollmentId, recordId, meetingKey)) {
      result.skipped += 1;
      continue;
    }
    ...
    if (!enrollment) {
      result.warnings.push(`Recording credit skipped: Enrollment ${enrollmentId} not found.`);
      result.skipped += 1;
      continue;
    }

    if (
      fieldExists(enrollmentsTable, CONFIG.enrollments.progressProcessingEnabled) &&
      !getBooleanish(enrollment, enrollmentsTable, CONFIG.enrollments.progressProcessingEnabled)
    ) {
      result.skipped += 1;
      continue;
    }

    const wasMatches = wasQueryRecords.filter(record =>
    ...
    if (wasMatches.length !== 1) {
      throw new Error(
        `Expected exactly one Weekly Athlete Summary for recording credit Enrollment ${enrollmentId} + Week ${weekId}; found ${wasMatches.length}.`
      );
    }

    const lastChance = findSourceKeyEvents(
      (
        await xpEventsTable.selectRecordsAsync({
    ...
    if (lastChance.length > 1) {
      throw new Error(
        `Multiple canonical recording XP Events found for Source Key ${sourceKey}. Record IDs: ${lastChance.map(r => r.id).join(", ")}`
      );
    }
    if (lastChance.length === 1) {
      result.skipped += 1;
    ...
    const payload = buildXpEventPayload({
      ...
      source: xpSourceLabel,
      bucketKey: CONFIG.xpLabels.bucketKey,
      points: xpAmount,
      reason: [
        CONFIG.xpLabels.recordingReasonPublic,
        `Enrollment ID: ${enrollmentId}`,
        `Zoom Meeting ID: ${recordId}`,
    ...
    if (fieldExists(xpEventsTable, "XP Reason Debug")) {
      payload["XP Reason Debug"] = `SC-147 ${CONFIG.version} ${sourceKey}`;
    }
  ...
  if (!result.actionOut) {
    result.actionOut = result.created > 0
      ? "created_recording_credit"
      : "reconciled_recording_pending_resolved";
  }
  return result;
}
...
async function runLiveLifecycleReconciliation(recordId) {
  const zoomRecord = await zoomTable.selectRecordAsync(recordId, {
    fields: buildFieldsToLoad(zoomTable, [
      ...
      CONFIG.zoom.recordingPendingTokens,
      CONFIG.lifecycle.currentSignature,
  ...
  const pendingTokens = parseRecordingPendingTokens(
    getText(zoomRecord, zoomTable, CONFIG.zoom.recordingPendingTokens)
  );
  const hasPendingRecording = pendingTokens.length > 0;
  ...
  if (
    ...
    !isAlreadyAwarded &&
    hasPendingRecording &&
    normalizeText(getText(zoomRecord, zoomTable, CONFIG.zoom.meetingStatus)) !==
      normalizeText(CONFIG.statuses.completed)
  ) {
    setFinalOutputs({
      ok: true,
      actionOut: "reconciled_awaiting_completed_for_recording",
      statusOut: CONFIG.outputStatuses.skipped,
      debugStep: "recording_pending_awaiting_completed",
      zoomMeetingId: recordId,
      errorOut: "Recording pending; Meeting Status is not Completed. Last signature not acknowledged.",
    });
    return;
  }
  ...
  if (lifecycleEnrollmentIds.length === 0) {
    if (hasPendingRecording) {
      const recordingResult = await processRecordingCreditsForMeeting({
        recordId,
        meetingKey,
        meetingStatus,
        meetingDateKey,
        ...
        enrollmentById,
        ruleQueryRecords: ruleQuery.records,
      });
      if (recordingResult.warnings.length) {
        throw new Error(`Partial writeback warning: ${recordingResult.warnings.join(" | ")}`);
      }
      if (recordingResult.awaitingCompleted) {
        setFinalOutputs({
          ok: true,
          actionOut: recordingResult.actionOut,
          statusOut: CONFIG.outputStatuses.skipped,
          debugStep: "recording_pending_awaiting_completed",
          zoomMeetingId: recordId,
      ...
      const { currentSignature: postRecordingSignature } =
        await readFormulaSettlement(recordId, startingSignature, {
          requireChangedSignature: recordingResult.formulaSignatureMustChange,
        });
      const remainingPending = parseRecordingPendingTokens(
        getText(
          await zoomTable.selectRecordAsync(recordId, {
            fields: buildFieldsToLoad(zoomTable, [CONFIG.zoom.recordingPendingTokens]),
          }),
          zoomTable,
          CONFIG.zoom.recordingPendingTokens
        )
      ...
      if (remainingPending.length > 0) {
        setFinalOutputs({
          ok: true,
          actionOut: recordingResult.actionOut || "reconciled_recording_still_pending",
          statusOut: CONFIG.outputStatuses.updated,
          debugStep: "recording_pending_unresolved",
          zoomMeetingId: recordId,
          zoomMeetingKey: meetingKey,
          weekId: weekIds[0],
          attendeeCount: 0,
      ...
      try {
        await updateRecordSafe(zoomTable, recordId, {
          [CONFIG.lifecycle.lastSignature]: postRecordingSignature,
          [CONFIG.zoom.createXpEvents]: false,
          [CONFIG.zoom.xpAwardError]: "",
        });
      } catch (error) {
        throw new Error(`Partial writeback warning: recording acknowledgement failed: ${error.message || error}`);
      }
      const acknowledgedRecording = await zoomTable.selectRecordAsync(recordId, {
        fields: buildFieldsToLoad(zoomTable, [CONFIG.lifecycle.reconciliationNeeded]),
      });
      if (
        getNumericValue(
        ...
        throw new Error("Recording acknowledgement did not settle Reconciliation Needed? to numeric 0.");
      }
      setFinalOutputs({
        ok: true,
        actionOut: recordingResult.actionOut || "reconciled_recording_credit",
        statusOut: recordingResult.created > 0
          ? CONFIG.outputStatuses.created
          : CONFIG.outputStatuses.updated,
        debugStep: "recording_lifecycle_acknowledged",
        zoomMeetingId: recordId,
        zoomMeetingKey: meetingKey,
        weekId: weekIds[0],
        attendeeCount: 0,
        baseEventsCreated: recordingResult.created,
      });
      return;
    }
  ...
  for (const enrollmentId of lifecycleEnrollmentIds) {
    ...
    const onRoster = attendeeIds.includes(enrollmentId);
    const enrollmentActive = getBooleanish(
      enrollment,
      enrollmentsTable,
      CONFIG.enrollments.active
    );
    const schoolYearMatch = Boolean(
      enrollmentSchoolYear && enrollmentSchoolYear === weekSchoolYear
    );
    /*
     ...
     * for withdrawal/restoration). Recording-only passes must not invent live XP.
     */
    const liveAwardsArmed = createXpEvents || isAlreadyAwarded;
    const eligible =
      liveAwardsArmed &&
      ...
      enrollmentActive &&
      schoolYearMatch;
    const shouldDeactivateOwned =
      normalizeText(meetingStatus) === normalizeText(CONFIG.statuses.cancelled) ||
      !onRoster ||
      !enrollmentActive ||
      !schoolYearMatch;
    ...
    if (!eligible) {
      if (
        shouldDeactivateOwned &&
        matching.length === 1 &&
        getBooleanish(matching[0], xpEventsTable, CONFIG.xpEvents.active)
      ) {
        try {
      ...
      if (shouldDeactivateOwned) {
        ...
        lifecycleAction = "reconciled_awaiting_completed_status";
      }
      continue;
    }
  ...
  const recordingResult = hasPendingRecording
    ? await processRecordingCreditsForMeeting({
      recordId,
      meetingKey,
      meetingStatus,
      meetingDateKey,
  ...
  if (recordingResult.warnings.length) {
    eventWarnings.push(...recordingResult.warnings);
  }
  if (recordingResult.formulaSignatureMustChange) {
    formulaSignatureMustChange = true;
  }
  if (recordingResult.created > 0) {
    baseEventsCreated += recordingResult.created;
    lifecycleAction = recordingResult.actionOut || "created_recording_credit";
  } else if (
    recordingResult.awaitingCompleted &&
    (lifecycleAction === "reconciled" ||
      lifecycleAction === "reconciled_awaiting_completed_status")
  ) {
    lifecycleAction = recordingResult.actionOut;
  }
  ...
  if (remainingPending.length > 0) {
    setFinalOutputs({
      ok: true,
      actionOut: lifecycleAction || "reconciled_recording_still_pending",
      statusOut: CONFIG.outputStatuses.updated,
      debugStep: "recording_pending_unresolved",
      zoomMeetingId: recordId,
      zoomMeetingKey: meetingKey,
  ...
  try {
    ...
    const shouldMarkAwarded =
      createXpEvents ||
      isAlreadyAwarded ||
      baseEventsCreated > 0 ||
      baseEventsUpdated > 0 ||
      baseEventsDeactivated > 0;
    const meetingUpdates = {
      [CONFIG.zoom.createXpEvents]: false,
      [CONFIG.zoom.xpAwardError]: "",
    };
    if (shouldMarkAwarded) {
      meetingUpdates[CONFIG.zoom.xpAwardStatus] = buildSingleSelectValueOptional(
        zoomTable,
        CONFIG.zoom.xpAwardStatus,
        CONFIG.statuses.awarded
      );
    }
    await updateRecordSafe(zoomTable, recordId, meetingUpdates);
...
function assertRequiredSchema() {
  ...
  if (fieldExists(zoomTable, CONFIG.zoom.recordingPendingTokens)) {
    requireField(
      zoomTable,
      CONFIG.zoom.recordingPendingTokens,
      "Zoom Meetings -> Recording Pending Reconcile Tokens"
    );
  }
  requireField(
    zoomTable,
