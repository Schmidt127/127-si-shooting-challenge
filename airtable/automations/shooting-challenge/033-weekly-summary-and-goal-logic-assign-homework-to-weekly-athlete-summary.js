...
function linkedCell(ids) { return [...new Set(ids)].map(id => ({ id })); }
function libraryDisplayName(record) {
  if (!record) return "";
  return getText(record, homeworkLibraryTable, CONFIG.homeworkLibrary.assignmentFullName) ||
...
async function main() {
  ...
  try {
    ...
    setOutputSafe("debugStep", debugStep);
    const enrollment = await enrollmentsTable.selectRecordAsync(enrollmentId, { fields: [CONFIG.enrollments.programInstance] });
    if (!enrollment) throw new Error(`Enrollment not found: ${enrollmentId}`);
    programInstanceId = oneLinkedId(enrollment, enrollmentsTable, CONFIG.enrollments.programInstance, "Enrollment.Program Instance");
    ...
    if (duplicateSlots.length) {
      throw new Error(`Multiple active PHA rows for same Program Instance + Week + slot: ${duplicateSlots.map(([slot, count]) => `${slot}(${count})`).join(", ")}`);
    }
    ...
    if (existingHomeworkIds.length === 0) {
      await weeklySummaryTable.updateRecordAsync(recordId, { [CONFIG.weeklySummary.homework]: linkedCell(matchedHomeworkIds) });
      homeworkWritten = true;
    } else if (!sameSet(existingHomeworkIds, matchedHomeworkIds)) {
      throw new Error(`Weekly Athlete Summary.Homework conflicts with current PHA truth. Existing=[${existingHomeworkIds.join(",")}], PHA=[${matchedHomeworkIds.join(",")}].`);
    }

    debugStep = "reconcile_deferred_homework_completions";
    setOutputSafe("debugStep", debugStep);
    const completionQuery = await homeworkCompletionsTable.selectRecordsAsync({ fields: Object.values(CONFIG.homeworkCompletions) });
    ...
    const deferred = completionQuery.records.filter(completion => {
      ...
      return enrollmentIds.length === 1 && enrollmentIds[0] === enrollmentId &&
    ...
    for (const completion of deferred) {
      await homeworkCompletionsTable.updateRecordAsync(completion.id, { [CONFIG.homeworkCompletions.weeklySummary]: [{ id: recordId }] });
      reconciledCompletionIds.push(completion.id);
    }

    debugStep = "load_titles";
    const libraryFields = [CONFIG.homeworkLibrary.assignmentFullName, CONFIG.homeworkLibrary.assignmentTitle]
    ...
    setOutputSafe("actionTaken", homeworkWritten ? "assigned_homework_and_reconciled" : reconciledCompletionIds.length ? "reconciled_deferred_completions" : "already_aligned");
    setOutputSafe("statusOut", "success");
    setOutputSafe("errorOut", "");
    setOutputSafe("debugStep", "complete");

    console.log(JSON.stringify({ automation: SCRIPT.scriptName, version: SCRIPT.version, weeklySummaryId: recordId, enrollmentId, programInstanceId, weekId, gradeBandIds, gradeBandSchedulingUsed: false, matchedHomeworkIds, homeworkWritten, reconciledCompletionIds, xpEventsCreated: 0 }));
  } catch (error) {
    ...
    console.log(JSON.stringify({ automation: SCRIPT.scriptName, version: SCRIPT.version, statusOut: "error", debugStep, errorOut: message }));
    throw error;
  }
}
