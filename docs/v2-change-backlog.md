...
| **C-023** | File dedup by **content hash**, not title/filename | **H3 PASS (2026-07-10)** — `rec1ZyqOfljt4foEX` same bytes as `recF86pJTIMFoEypJ`; duplicate flags written (`Exact Duplicate`); **flag-only** (upload not blocked; new S3 key). **Not closed** — Mike decision: block vs reuse vs flag-only for Production. [H3 doc](./deploy-checklists/C-023-dev-h3-duplicate-bytes-test.md) | C-013, C-024 | **in progress** |
