...
| **C-025** | Zoom **recording** attendance — partial credit path | **Owner decisions APPROVED (2026-07-13 S16).** **DEV formulas/conflict APPLIED (2026-07-13).** **DEV deadline true-date APPLIED (2026-07-14)** — `Week End Date` lookup + patched `Calculated Recording Quiz Deadline` (date result) + view `Zoom Recording Quiz - Past Deadline` present; Schmidt **4/4**. Config linkage + automations **designed S17** ([config linkage](./deploy-checklists/C-025-config-linkage-design.md), [deadline](./deploy-checklists/C-025-deadline-repair-design.md), [automations](./deploy-checklists/C-025-automation-packages-stage17.md)). **Next:** DEV Config field create + Effective linkage; then paste 117a–f (no PROD). | C-024, V2-006, V2-005 | deadline done / Config schema blocked |
