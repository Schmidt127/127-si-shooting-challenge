...
> **Historical — read-only (DEV base retired 2026-08-19).** The separate Airtable DEV base (`appTetnuCZlCZdTCT`) is no longer used. Current operation is **production-only** (`appn84sqPw03zEbTT`) with controlled PROD testing per [CURRENT-TRUTH.md](../CURRENT-TRUTH.md) and [04-ai-development-standards.md](./04-ai-development-standards.md). Do not execute live writes against the retired DEV base. Offline fixtures in `tools/airtable/v2_dev_runbook/` remain valid for repository contract tests.
