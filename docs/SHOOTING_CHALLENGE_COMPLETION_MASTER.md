...
| SC-032 | Config | Season settings (dates, windows) | Built in Repository | Challenge-Year engine: Config contract + Week generator/validator + Activity Date resolver + preflight/manifest (Agent 2 online 2026-07-24); extends config-selection + enrollment-season | Seed real next-year Weeks via generated CSV; wire proposed Config date fields only if Mike authorizes schema | SC-065, SC-084 | 005 date mapping; never hard-code current year | `docs/challenge-year/`; `lib/challenge-year/`; C-018; V2-013 | Approve Week 0 Sunday + regular week count | P1 | 2026-07-24 |
...
| SC-065 | Enrollment | Challenge dates / Weeks configuration rebuilt | Built in Repository | Weeks seed spec + template + read-only validator; **Challenge-Year Week generator** (Week 0..N + Post-Challenge, Sunday–Saturday, CSV/MD/JSON) + validation + dry-run missing-weeks preview | Manually import generated Weeks CSV in PROD with approved dates; link Program Instance | SC-032 | Denver timezone; Weeks remain manual (no auto-create automation) | `docs/challenge-year/`; `docs/online-agents/enrollment-season/`; C-018; 005 | — | P0 | 2026-07-24 |
...
| SC-067 | Enrollment | Program Instance multi-year design | Deferred | V2-013 decided direction; investigation 2026-07-05; **Challenge-Year rollover engine** now provides repo-side preflight/manifest/activation runbook without implementing full PI schema wave | Dedicated PI schema/linking wave later; use engine for annual ops | SC-032, SC-046 | Config changes must not rewrite history | V2-013; `docs/challenge-year/` | When to schedule wave? | P3 | 2026-07-24 |
