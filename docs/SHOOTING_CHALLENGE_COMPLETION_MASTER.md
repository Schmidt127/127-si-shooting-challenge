...
| SC-031 | Config | Weekly schedule settings (build/send timing) | Live Tested in PROD | **118/119 ON** Sun 5:00/10:00 AM Denver; C-011 design | Keep dryRun/sendMode/includeSchmidt season-safe; paste 118/119 v1.5 | SC-051 | Mass email if Live+includeSchmidt | C-011; Agent 2 data-model | Confirm season inputs | P1 | 2026-07-24 |
...
| SC-035 | Weekly Summary | Guaranteed Weekly Athlete Summary for every enrollment × ended week | Live Tested in PROD | **118 ON** + **072 v4.0** `send_short`; creators **031**+**118**+**101** resolved in Agent 2 WAS-CREATOR-RESOLUTION | Paste 118 v1.5; season `dryRun=false` `sendMode=Live` `includeSchmidt=false` | SC-004, SC-082 | Bounded create concurrency residual | `docs/next-wave/data-model/WAS-CREATOR-RESOLUTION.md` | **DECIDED `send_short`** | P0 | 2026-07-24 |
...
| SC-038 | Weekly Summary | Automatic package build (no Build checkbox) | Live Tested in PROD | **118 ON** arms Build; **072 v4.0** short empty-week package (`built_short_empty_week`, `packageKind=short_no_activity`) | Keep `allowSchmidtInput=false` post-test; season dryRun=false | SC-035 | 072 does not call Make | `docs/next-wave/was-email/WAS-WEEKLY-EMAIL-ARCHITECTURE.md` | Schedules ON | P0 | 2026-07-24 |
| SC-039 | Weekly Summary | Automatic send (no Send checkbox) | Live Tested in PROD | **119 ON** `send_armed` → **074** → Make Bulk Email May 18 → Gmail; Live writeback PASS | Keep 074 sendMode Live; paste 119 v1.5 | SC-038 | **119 does not post webhook**; Make owns Sent? + Make Send Status + Weekly Summary Sent At | `docs/next-wave/data-model/SENT-FIELD-OWNERSHIP.md` | Schedules ON verified | P0 | 2026-07-24 |
...
**Next package:** `docs/next-wave/data-model/MIKE-ACTIONS.md` — paste 118/119 v1.5 season inputs; Week Code OMNI attest; Fillout 2026–2027 checklist. Schedules already ON (verified-prod).
