...
|-------|--------|
| Created | 2026-07-23 |
| Last updated | 2026-07-24 (continuation: 118 v1.5 Live-season arming; schedules ON confirmed; season input Mike checks) |
...
| SC-031 | Config | Weekly schedule settings (build/send timing) | Live Tested in PROD | **118/119 schedules ON** Sun 5:00/10:00 AM America/Denver; activation authorized + Live email/writeback proven; repo **118 v1.5** Live-season arming | Confirm UI season inputs `dryRun=false` + 118 `sendMode=Live`; paste 118 v1.5 if PROD still v1.4; monitor first live Sunday | SC-051 | Do **not** disable schedules; v1.4 hardcoded WAS Test + refused Live+!dryRun | `WAS-WEEKLY-EMAIL-ARCHITECTURE.md`; `go-live/MIKE-ACTIONS.md` | **Authorized ON** | P0 | 2026-07-24 |
...
| SC-035 | Weekly Summary | Guaranteed Weekly Athlete Summary for every enrollment × ended week | Live Tested in PROD | **118 ON** (Sun 5 AM); repo **v1.5** Live-season arming (writes WAS sendMode from input); Schmidt WAS ensure + **072 v4.0** `send_short` delivered; hybrid creators **031**+**118**+**101** | Paste **118 v1.5** if PROD still v1.4; set season `dryRun=false` + `sendMode=Live`; monitor WAS uniqueness | SC-004, SC-082 | Schedule ON — do not revert OFF; never includeSchmidt+Live | `WAS-WEEKLY-EMAIL-ARCHITECTURE.md`; `go-live/MIKE-ACTIONS.md` | **DECIDED `send_short`**; schedules authorized | P0 | 2026-07-24 |
...
| SC-038 | Weekly Summary | Automatic package build (no Build checkbox) | Complete | **118 ON** arms Build; **072 v4.0 ON** built short empty-week package; season schedule active | Keep `allowSchmidtInput=false` for normal traffic; monitor empty-week `send_short` | SC-035 | 072 does not call Make | `docs/next-wave/was-email/WAS-WEEKLY-EMAIL-ARCHITECTURE.md` | — | P0 | 2026-07-24 |
| SC-039 | Weekly Summary | Automatic send (no Send checkbox) | Complete | **119 ON** (Sun 10 AM) arms Send → **074** webhook → Make Bulk Email May 18 → Gmail; **074 sendMode=Live** writeback PASS | Keep 074 sendMode Live; monitor first live Sunday send volume | SC-038 | **119 does not post webhook**; Make owns Live Sent? writeback; never force 074 Test | `docs/next-wave/was-email/WAS-WEEKLY-EMAIL-ARCHITECTURE.md` | Schedules authorized ON | P0 | 2026-07-24 |
...
| SC-045 | Weekly Summary | Welcome, homework, video, Zoom, and weekly emails all work | Installed in PROD | Weekly WAS Live path PASS 2026-07-24 (`send_short` + writeback); schedules ON; 071–077 + 117f still need individual re-proof | Re-test homework/video/welcome templates; 117f go-live; confirm 118 v1.5 season inputs | SC-039, SC-124 | Webhooks not in git | automation-index; WAS architecture; go-live/MIKE-ACTIONS | — | P0 | 2026-07-24 |
...
**Next package:** UI attestation (112 / 063/111 / 117 XOR 117c / Automations table re-export) + Schmidt re-test packs for installed-but-unproven pipelines. Weekly email schedules are **already ON** — do not re-authorize as OFF.
