...
|-------|--------|
| Created | 2026-07-23 |
| Last updated | **2026-07-25** (Repo completion agent: SC-049 Weekly Threshold writer 035 rebuilt; 057 Denver date fix v1.4; SC-002 fixtures SCN-021–026; total 147) |
...
| SC-002 | Testing | Test scenario library / templates for repeatable suites | Built in Repository | Machine-readable catalog: **26 fixtures** + README + `catalog.json` (SCN-001–026) incl. HW/Video/Zoom/Weekly Threshold rows | Optional Airtable Scenario Library table later; live-execute new fixtures on Schmidt | SC-001 | Library is config, not a second XP path | `docs/testing/scenarios/` | Confirm Airtable library table still wanted | P1 | 2026-07-25 |
...
| SC-021 | Config | Config-over-code audit (no hardcoded season numbers in scripts) | Built in Repository | Hardcode audit + helpers; year-aware Config resolver; **054 v5.6** + **066 v3.3** in PROD; **057 v1.4** Denver date-key fix in repo (UTC ISO slice removed) | Paste **057 v1.4**; migrate remaining consumers to resolver; finish remaining literals | SC-022 | Changing options breaks scripts | `docs/next-wave/config-selection/`; `CONFIG-HARDCODE-AUDIT.md`; MIKE-ACTIONS #2 | — | P0 | 2026-07-25 |
...
| SC-049 | Data Integrity | Duplicate-prevention keys documented and audited | Built in Repository | Full XP source catalog + JSON; Schmidt live Source Keys unique; **Weekly Threshold writer rebuilt as 035 v1.0** (`WEEKLY_THRESHOLD\|{enr}\|{week}\|{percent}`); offline contract tests PASS | Mike UI-attest no competing Threshold automation; paste 035; Schmidt live HW/video/Zoom/threshold key proofs | SC-046 | Reruns must be safe; do not dual-ON old Threshold automation | `035-…weekly-threshold…js`; `docs/deploy-checklists/035-weekly-threshold-xp-v1.0.md`; `weekly-threshold-xp.test.js` | Attest + paste 035 | P0 | 2026-07-25 |
...
|---------|--------|----------|
| **SC-049 / XP-D1 Weekly Threshold writer** | Built in Repository | New **035** v1.0; contracts + `weekly-threshold-xp.test.js`; registry updated; deploy checklist; PROD paste + Schmidt live still required |
| **SC-021 / 057 Denver date keys** | Built in Repository (paste pending) | **057 v1.4** `getDateKeyFromDateOnly` uses America/Denver Intl; `xp-date-normalization.test.js` PASS |
| **SC-002 scenario fixtures** | Built in Repository | Catalog expanded to **SCN-021–026** (HW / Video / Zoom / Weekly Threshold) |
| Levels blank/idempotent tests | Tests only | `overnight-level-gate-boundaries.test.js` blank stats + duplicate recalc |

**Not claimed Complete:** No PROD Airtable paste or live Schmidt proof in this session.  
**Avoided:** Communications base / COM-MAKE-001; inventing Manual Bonus writer; dual-writer Threshold install without Mike attest.

**Next package (recommended):** Mike UI-attest + paste **035** + **057 v1.4**; Schmidt threshold + Perfect Week live proof; then HW/video/Zoom live packs from SCN-021–024.
