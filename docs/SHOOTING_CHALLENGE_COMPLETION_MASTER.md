...
| `PKG-003` | Reconcile remaining stale operational/status references after each approved package | `needs repo work` | §4 SC-139; [Authority Map](./AUTHORITY-MAP.md) | `PKG-001` and later package evidence | Cursor | Audit pass, corrected active wording, preserved historical notices, and changed-file traceability | 2026-08-10 — Execution matrix IDs advanced: PKG-003 (user-assigned proof-gap design) — specification `docs/investigations/PKG-003-CONTROLLED-PROOF-SPECIFICATION.md`; existing matrix label describes SC-139 stale-reference reconciliation, so the ID conflict remains explicitly unresolved | Repo | Mike resolves the PKG-003 label conflict before promoting this proof specification or starting SC-139 reconciliation | 3 |
