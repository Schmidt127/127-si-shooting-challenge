...
|-------|--------|
| Created | 2026-07-23 |
| Last updated | **2026-08-12** (Automation 076 v8.4 cleaned-recipient hotfix; no Production change) |
...
### Dashboard reconciliation (2026-08-12 — Automation 076 v8.4 cleaned-recipient hotfix)

| Item | Status | Evidence |
|------|--------|----------|
| Automation **031 v4.0 / 076 v8.4** | **Built in Repository; Production replacement pending** | 031 remains the approved authoritative normal-athlete-activity WAS creator. 076 v8.4 retains the verified `Program Instance - Sync` table and now requires the cleaned Enrollment `Parent Email - Cleaned` as the authoritative parent recipient; raw `Parent Email` is not a fallback. Cleaned athlete recipients are optional and deduplicated case-insensitively. Queue behavior, readiness guards, dynamic `recordId`, and deterministic replay remain intact. `rec58gdymfPKKeVRI` is temporary manual-test-only. Paste order is 076 v8.4 first, then 031 v4.0. [`AUTOMATION-031-PASTE-AND-TEST-PACKET.md`](./prod-completion/2026-08-07/AUTOMATION-031-PASTE-AND-TEST-PACKET.md) · [`PKG-006-DAILY-SUBMISSION-HUB-HANDOFF.md`](./deploy-checklists/PKG-006-DAILY-SUBMISSION-HUB-HANDOFF.md) |
| Focused validation | **PASS — 031 28/28; 076 v8.4 focused suite; 010 9/9** | 031 tests: **28/28 passed**; 076 focused tests cover cleaned-parent success, raw-parent rejection, case-insensitive recipient deduplication, invalid/missing parent fail-closed behavior, and deterministic replay; 010 tests: **9/9 passed**; 076 canonical contracts passed; syntax passed; `git diff --check` passed; IDE diagnostics reported no errors. Repository ESLint remains unavailable because no `eslint.config.*` exists; broad lint is not claimed. |
...
| **Status** | **Repository-ready / controlled Production promotion blocked pending trigger-owner/timing verification** — 076 v8.4 cleaned-recipient correction complete; no Production paste performed |
