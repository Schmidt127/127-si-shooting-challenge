...
**Status:** Repo deliverables complete · live Make/AWS smoke **blocked** without DEV credentials in agent env  
...
**Hard stops:** DEV `appTetnuCZlCZdTCT` only · 070a OFF · no PROD · no `070a-*.js` edits · no C-023 implementation · no deletes
...
|---|--------|--------|--------|
| 1 | Ensure cloud/local agent has DEV `.env`: `AIRTABLE_TOKEN`, `MAKE_DEV_UPLOAD_WEBHOOK_URL`, `UPLOAD_WEBHOOK_SECRET`, `LAMBDA_FUNCTION_URL`, optional AWS keys | Local ops | ☐ Blocked if missing |
| 2 | Confirm DEV Lambda `ALLOW_ROUTE_KEYS=video_feedback,homework_completion` | AWS DEV | ☐ |
| 3 | Patch Make Module 2 to accept `070a` + `homework_completion` (same HTTP→Lambda chain) | Make DEV | ☐ |
