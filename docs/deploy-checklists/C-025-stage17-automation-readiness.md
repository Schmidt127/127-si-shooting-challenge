...
**CONFIRMED:** Production lacks the `Zoom Attendance` table and the recording XP Source option, so Stage 17 automations cannot be safely pasted or enabled. See [schema gap analysis](./C-025-stage17-prod-schema-gap-analysis.md), [implementation checklist](./C-025-stage17-prod-implementation-checklist.md), and [isolated smoke test](./C-025-stage17-prod-smoke-test.md).
...
| **117f — Send Approval Email** | `Zoom Attendance`; exact trigger must target satisfactory eligible recording rows | `recordId` required | `117f-zoom-recording-send-approval-email.js` v1.1.0 | ZA | ZA sent-at field after successful send | satisfactory/approval/conflict, effective email config, send key; `ZOOM_REC_APPROVAL\|{enrollmentRID}\|{meetingRID}` | Standard outputs; missing/disabled config or webhook skips safely. |
