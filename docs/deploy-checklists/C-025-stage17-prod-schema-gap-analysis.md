...
| Architecture | PROD = **pre–Stage 17** live attendance; Production = Stage 17 ZA + Config/ZM effective fields |
...
| Raw Production−PROD inventory delta (includes Testing Scenarios + ZZZ archives) | **233** (audit script) |
...
| Fields on Homework Completions for Zoom recording? | **None** in PROD or Production |
...
## 3. Production-to-PROD table mapping

| Table | Production | PROD | Notes |
|-------|:---:|:----:|-------|
...
| **Testing Scenarios** | Yes | **No** | Production-only ETF harness — **not** required for PROD 117/057/042 runtime |
| Engineering Test Framework | No | No | Not a table name; ETF uses **Testing Scenarios** in Production |
| SYNC - Brackets | Yes | No | Unrelated to Stage 17 |

**Production-only tables:** `Zoom Attendance`, `Testing Scenarios`, `SYNC - Brackets`
**PROD-only tables:** none

**Table counts:** Production **32** · PROD **29**

---

## 4. Production-to-PROD field mapping

### 4.1 Zoom Attendance (Production canonical → PROD)

| Design / doc alias | Production field | PROD | Type (Production) |
|--------------------|-----------|------|------------|
...
| XP Event link | *(no ZA→XP Events link field in Production)* | n/a | — |
...
**Full Production Zoom Attendance inventory:** **40 fields** (all absent in PROD because the table is absent). Automation-critical subset (~22) includes Method, links, RIDs, review/satisfactory/timestamps, credit formulas, XP amount, gate/PW flags, email send key/sent-at.

### 4.2 XP Events

| Requirement | Production | PROD |
|-------------|-----|------|
...
**Production recording sample:** at least one `ZOOM_CREDIT|…` at **30** XP.
...
| ZZZ C025 Archive* | Production only | Do **not** migrate archives to PROD |

### 4.4 Config (recording)

Production has Stage 17 recording controls; PROD has **none** of the recording Config set. Observed Production value: `Zoom Recording XP Percent of Live` = **50** with `Recording Path Enabled?` = true on at least one Config row.
...
| Zoom Attendance fields | 40 | Entire Production inventory |
...
- `Testing Scenarios` table + 31 fields (Production ETF only)
- `SYNC - Brackets`
...
Once the Zoom Attendance table is created, formulas must be rebuilt carefully (Production formulas reference field IDs). Treat formula text / linked lookup wiring as a **manual OMNI verification** item (§17).
...
| View | Where | PROD |
|------|-------|------|
| `Zoom Recording Quiz - Past Deadline` | Zoom Attendance (Production) | **Missing** (table missing) — recreate after table |
...
| Config row values (percent 50, path/makeup/gate/PW toggles) | **Yes** before enabling recording awards | Match Production global/program intent |
...
2. **OMNI: Config** — add Stage 17 recording fields; set percent **50** and path/makeup/gate/PW/email/deadline to match approved Production intent.
...
4. **OMNI: Create `Zoom Attendance`** — fields, selects, formulas, lookups matching Production; verify credit key / conflict / amount (=30 when base 60 × 50%).
...
| — | **115** ETF | Production-only | Do **not** promote to PROD as part of Stage 17 athlete path |
...
- Production one-click / downstream verification still failing (see existing PROD promotion STOP docs)
...
2. **Formula correctness after recreate** — Production formulas use field IDs; PROD rebuild must be visually confirmed in OMNI.
...
- [ ] Production Stage 17 one-click / downstream verification **Pass** (separate gate)
...
| Base | Touched? |
|------|----------|
| Production `appn84sqPw03zEbTT` | **Untouched** (read-only) |
