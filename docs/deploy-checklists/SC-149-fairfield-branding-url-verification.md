...
| # | Action | Done |
|---|--------|------|
| 1 | Vercel → confirm Production `NEXT_PUBLIC_LANDING_URL` = `https://www.fairfieldbasketballclub.com` | [ ] |
| 2 | Vercel → confirm Production `NEXT_PUBLIC_SITE_URL` = `https://www.fairfieldbasketballclub.com/shoot` | [ ] |
| 3 | Vercel → confirm `NEXT_PUBLIC_BASE_PATH` = `/shoot` (or unset if project relies on repo default) | [ ] |
| 4 | Merge approved GitHub branch to `master` (if not already) | [ ] |
| 5 | Trigger or wait for Vercel Production deploy from `master` | [ ] |
| 6 | Run production smoke (below) | [ ] |
| 7 | Visual check: logo + footer + “Home” hub link → Fairfield landing (not Hoop Challenges) | [ ] |
| 8 | Update backlog: SC-149 → **Live Tested in PROD**; MRW-E02 → **COMPLETE** | [ ] |
...
| Check | Expected | Done |
|-------|----------|------|
| Playwright `production-smoke.spec.ts` | Pass; logo/landing links `href="https://www.fairfieldbasketballclub.com"`; zero `hooopchallenges` anchors | [ ] |
| HTTP smoke `http-smoke.mjs` | Pass; home HTML contains Fairfield landing href; no `hoopchallenges` host strings | [ ] |
| View page source on `/shoot/` | No `hooopchallenges` or `hoopchallenges.com` in anchor hrefs | [ ] |
| In-app nav (`/shoot/leaderboard`, etc.) | Stays on `/shoot/*`; does not redirect to landing root | [ ] |
