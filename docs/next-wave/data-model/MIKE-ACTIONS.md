...
## P0 — Paste **118 v1.5** (required functional fix)

GitHub `118` v1.5 allows `sendMode=Live` with `dryRun=false` and writes WAS `sendMode` from input (no longer hardcodes Test / no longer refuses Live+`dryRun=false`).

1. Paste **118 v1.5** into PROD automation (skip GitHub header).  
2. Set 118 inputs: `dryRun=false`, `sendMode=Live`, `includeSchmidt=false`, `emptyWeekPolicy=send_short`.

### 119 — paste **not required** for Live season

**Proof:** vs PROD/master v1.4, `119` v1.5 changes only GitHub/docblock text, `CONFIG.version` (+ metadata), and unused `weeks.weekCode` field name in the Weeks field list. Arming logic (`Send to Make?`, Ready/Sent gates, dryRun behavior) is unchanged. Docblock itself says “Docs only for ownership clarity.”

3. **Optional:** paste 119 v1.5 for version-string alignment only.  
4. **Required regardless of paste:** set 119 inputs `dryRun=false`, `includeSchmidt=false` (leave schedule **ON**).
