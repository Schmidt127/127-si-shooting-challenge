...
| 3 | `edit_submission_parent` | **Target:** `https://forms.fairfieldbasketballclub.com/shoot-editsubmission?id={SubmissionRecordId}` · **Legacy:** `https://form.fillout.com/t/vNgeHardYcus?id=…` | `vNgeHardYcus` | `Submissions` (edit prefill) | Parent edit/resubmit linked from daily receipt emails | Mike / Fillout admin | Formula field `Edit Submission - Parent` on Submissions | **Y** | Mike decision #6: migrate to custom domain; update formula per [FUT-039-fillout-branding.md](../../deploy-checklists/FUT-039-fillout-branding.md) |
...
| 5 | `homework_fillout_per_assignment` | **Not created** (FUT-029 historical Fillout idea) | — | TBD (`Homework Completions` spine) | Optional online homework answers (superseded architecture) | Future — only if separately authorized as fallback under FUT-029 | Design only; **no live forms**; canonical direction is in-app intake adapter | **N** | See [`FUT-029-GRADE-BAND-HOMEWORK-PLATFORM-PLAN.md`](../homework-pipeline/FUT-029-GRADE-BAND-HOMEWORK-PLATFORM-PLAN.md) |
...
| Edit submission formula | Submissions.`Edit Submission - Parent` → **target** `"https://forms.fairfieldbasketballclub.com/shoot-editsubmission?id=" & RECORD_ID()` · **legacy** `form.fillout.com/t/vNgeHardYcus` |
...
| 2026-09-01 | Edit submission target URL + Mike decisions (brief §8) |
