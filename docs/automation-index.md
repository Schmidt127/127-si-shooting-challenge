...
| **076** | Email — Create Daily Submission Communications Hub Handoff | Existing `Build Daily Email Now?` signal, with Counted/Stat Mode supporting guards; upstream owner and post-031 timing require Mike verification | `076-email-notifications-and-external-handoffs-build-daily-submission-email-package.js` (**v8.1** — stages one `DAILY_SUBMISSION` Email Handoff Queue row, resolves exact-key conflicts, then promotes one row to Ready; Hub owns content) |
