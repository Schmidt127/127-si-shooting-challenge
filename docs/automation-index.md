...
| **076** | Email — Create Daily Submission Communications Hub Handoff | Consumes the `Build Daily Email Now?` signal checked only by 031 after counted/stat-mode and final summary validation; 076 clears it after queue create/reuse | `076-email-notifications-and-external-handoffs-build-daily-submission-email-package.js` (**v8.1** — stages one `DAILY_SUBMISSION` Email Handoff Queue row, resolves exact-key conflicts, then promotes one row to Ready; Hub owns content) |
