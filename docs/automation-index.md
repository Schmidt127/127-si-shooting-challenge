...
| **071** | Homework parent feedback Hub handoff | Parent Feedback Ready? and not yet sent | GitHub Hub queue create (**v4.1**); delivery Hub → Resend via **079** |
...
| **057** | Achievements — Calculate Perfect Week Eligibility | WAS Perfect Week recalc | `057-achievements-and-milestones-calculate-perfect-week-eligibility.js` (**v2.2** — Config-driven Perfect Week video minimum (SC-034); inactive enrollment and unsettled/multiple/wrong-scope goals fail closed; requires lookup parity with the linked active goal) |
...
| **078A** | Enrollment — Create WELCOME Email Handoff | Enrollments after Athlete + cleaned parent email + Program Instance | `078A-email-notifications-and-external-handoffs-enrollment-create-welcome-email-handoff.js` (**v1.4** — queue create only; `WELCOME\|ENROLLMENTS\|{Enrollment Id}`; does **not** write retired 075 Enrollment fields) |
...
| # | Airtable automation name | Trigger | File |
|---|--------------------------|---------|------|
| **101** | Zoom Attendance XP — Award Meeting XP | Zoom Meetings when `Zoom XP Reconciliation Needed? = 1`; dynamic `recordId`; no Create XP Events, Attendees, or Completed as primary condition | `101-zoom-attendance-xp-award-meeting-xp.js` (**v6.8** Production Live / SC-147 verified 2026-09-02). Live Source Key `ZOOM_ATTEND_BASE\|{Zoom Meeting Key}\|{Enrollment RID}`; recording `ZOOM_RECORDING_CREDIT\|{Enrollment RID}\|{Zoom Meeting RID}` (half live XP; not Perfect Week; mutually exclusive with live). **No Automation 121.** 031 is the sole WAS creator |
