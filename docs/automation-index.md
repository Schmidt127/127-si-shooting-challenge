...
| **074** | Email — Send Weekly Summary Email Package to Make | WAS Ready? + !Sent? + Send to Make? + package fields | `074-…-send-weekly-summary-email-package-to-make.js` (**v2.2**) — **posts webhook**; READs Sent? to block dupes; does **not** write Sent?; emits `statusOut`/`debugStep`; **ON**; PROD **sendMode=Live** (or blank+WAS Live) — **never** fixed Test |
