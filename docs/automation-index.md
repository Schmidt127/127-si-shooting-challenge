...
| 058 | Achievements — Create Perfect Week Unlock | Lifecycle-capable WAS trigger; dynamic `recordId` | `058-achievements-and-milestones-create-perfect-week-unlock.js` (**v1.6** — SC-153 lifecycle deactivate/restore; writes Unlocks **Milestone Source Key** + **Coach Note**; identity `PERFECT_WEEK|{enr}|{week}`; UI trigger paste per SC-152/153 checklist) |
