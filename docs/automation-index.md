...
| **057** | Achievements — Calculate Perfect Week Eligibility | WAS Perfect Week recalc | `057-achievements-and-milestones-calculate-perfect-week-eligibility.js` (**v2.5** — SC-160 early HW counts for assigned-week Perfect Week; late still excluded; SC-152 clears `Perfect Week Recalc Needed?` on writeback; Config-driven Perfect Week video minimum (SC-034); inactive enrollment and unsettled/multiple/wrong-scope goals fail closed; requires lookup parity with the linked active goal) |
