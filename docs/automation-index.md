...
Live attendance XP remains **101**. **SC-147** recording half-XP is implemented in **101 v6.7** (GitHub merged PR **#338**) — no slot **121**. Recording approval email remains **117** (email only). Production paste pending Mike — disposable Production proof only ([`SC-147-zoom-recording-half-xp.md`](./deploy-checklists/SC-147-zoom-recording-half-xp.md); no DEV base).
