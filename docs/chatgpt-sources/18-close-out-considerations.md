...
| C-020 | 2026-07-03 | Testing / intake | **Test Intake harness (production-identical)** — **priority DEV build after 066** | **Test Intake** table + automation/extension: `Scenario Type`, `Run Test?` / `Dry Run?`, Enrollment (Schmidt or DEV test enrollments), shot fields, HW/video attachments → Fillout-shaped Submission with Enrollment pre-linked → **unchecks trigger**. **No test metadata on pipeline rows.** Multi-file video parity. Downstream automations **005–022**, **013**, **070a/b** documented. DEV first. See [testing-and-intake-architecture.md](./testing-and-intake-architecture.md) § C-020 | Design table + script on DEV; Testing views; promote after DEV pass | open |
...
| **C-020** | **High** | **Test Intake harness** — **next DEV build after 066**; production-identical runs; Scenario Type; multi-file video; Testing views by Enrollment; downstream automation map |
