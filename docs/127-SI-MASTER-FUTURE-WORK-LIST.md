...
| **SC-163** | Enrollment | Enrollments Goal Met Date reliability + backfill | P1 | **PARTIALLY COMPLETE — MIKE UI ACTION REQUIRED** (2026-09-05) | — | Ownership moved to **066 v4.0** (122 SUPERSEDED — capacity full). Convert lookup→date + paste 066 v4.0 + backfill still open. Checklist [`deploy-checklists/SC-163-goal-met-date.md`](./deploy-checklists/SC-163-goal-met-date.md). Prior PR **#438**; follow-up PR on `feature/sc-163-066-goal-met-date`. **Not live-complete.** |
