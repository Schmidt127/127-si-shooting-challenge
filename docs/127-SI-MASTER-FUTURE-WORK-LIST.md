...
| **SC-112** | Website | Athlete auth + dashboard | P2 | **MERGED/DEPLOYED** (magic-link **works**) · multi-child **CODE-FIXED** (select 404) · **NEEDS-PRODUCTION-DEPLOY + MIKE VERIFY** | SC-149 | Private `/shoot/dashboard`; opaque selection keys. **2026-09-04:** fixed child-select `router.push` double-`/shoot` 404 (`/shoot/shoot/dashboard`). Code complete on branch; Production deploy + Mike multi-child walkthrough still required. Closeout: [`audits/SC-112-finalization-closeout-20260903.md`](./audits/SC-112-finalization-closeout-20260903.md) |
...
| **SC-151** | Website | Family Dashboard allows registered Gmail parent emails | P1 | **In progress (this PR)** | SC-112 | Remove incorrect Gmail domain block + fix sign-in copy. **SC-112 remains closed.** Audit: [`audits/SC-151-family-dashboard-gmail-access-20260904.md`](./audits/SC-151-family-dashboard-gmail-access-20260904.md) |
...
| Item | Status | Evidence |
|---|---|---|
| **SC-112 magic-link + private dashboard** | **MERGED/DEPLOYED** + **works** | PRs **#350–#357**. Tip SHA **`9a68281e`**. Magic-link operational with Production auth **on**. |
| **SC-112 multi-child parent auth** | **CODE-FIXED** (select→dashboard 404) · **NEEDS-PRODUCTION-DEPLOY + MIKE VERIFY** | PR **#373** multi-child keys. **2026-09-04:** JSON `redirectTo` was `/shoot/dashboard` then `router.push` doubled basePath → `/shoot/shoot/dashboard` Page Not Found. Fix: app-relative `redirectTo` + `toAppRouterHref`. Authenticated select/switch/sign-out still needs Mike Production walkthrough after deploy. |
...
| **SC-112 docs closeout (Agent 4)** | **Updated post-#381/#382/#383** — tip `9a68281e` | [`audits/SC-112-finalization-closeout-20260903.md`](./audits/SC-112-finalization-closeout-20260903.md) · hygiene archive `tools/season_simulation/_archive/session-20260903/` |
