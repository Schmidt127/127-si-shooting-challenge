...
- [ ] Work in **Production** first unless Mike authorizes a named PROD check.
...
1. Export current schema snapshot for the target base (Production, then PROD when authorized).
...
1. Hit `/shoot/tutorials`, `/shoot/shoutouts`, `/shoot/articles` against Production data (or mocked).
...
4. Run website smoke after rollback drill in Production.
