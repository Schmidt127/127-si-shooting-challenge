...
**Branched from:** current working-tree HEAD `147b5f73` (see note below)
...
> **Branch-base note.** The task named `master` as the default branch. `master` is **checked out in a linked worktree** (`…/127-si-shooting-challenge-integration`) and the primary working tree had uncommitted line-ending/content churn, so this audit branch was created from the current HEAD (`147b5f73`, tip of `feature/c025-stage17-zoom-attendance`) to avoid a disruptive checkout. All merge analysis below is computed against `origin/master` regardless of branch base.
...
| `chatgpt-recovery-2026-07-14/` | **2→3** | Recovery snapshot (91 files, ~0.85 MB): duplicate copies of `docs/`, `AGENTS.md`, repo-state text. Governed by existing `docs/UNTRACKED-RECOVERY-TRIAGE.md`. | Low — duplicates of `master` content; keep offline backup. | Keep local / move to external archive per triage doc §2. Do not commit. Ignored (Section 8). | No | Referenced conceptually by `docs/UNTRACKED-RECOVERY-TRIAGE.md` |
...
### 4.2 Remote branches merged into `origin/master` — safe to prune on remote (owner approval required; do not push in this run)
...
14 worktrees are registered:

```
…/127-si-shooting-challenge                 (this audit branch)      ACTIVE
