...
- Airtable Meta API **cannot DELETE fields** with current PAT (`DELETE` → 404). Fields are quarantined (`ZZZ DELETE — …` + description) for Mike UI trash.
...
| --- | --- | --- | --- |
| Quarantine + `BLANK()` | Homework Completions / `ZZZ DELETE — Submission Asset Review Summary (invalid legacy)` | `fldHchlovIaPlGKLk` | Formula valid; awaiting Mike UI delete |
...
| Quarantine | Levels / `ZZZ DELETE — Enrollments 3 (accidental empty)` | `fldTzIGODB2e03rvE` | Empty; no formula deps |
| Quarantine | Streak Occurrences / `ZZZ DELETE — Challenge / Season (empty backfill)` | `fldltgFPGVXHwRj4X` | Empty backfill helper |
| Quarantine | Streak Occurrences / `ZZZ DELETE — Backfill Run Label (empty backfill)` | `fldBFDl629arXFcnp` | Empty backfill helper |
| Quarantine | Achievements / `ZZZ DELETE — Uses Grade Band Scaling? (unused empty)` | `fldkIzG5emvUBQ0Tw` | Empty unused checkbox |
...
- Snapshot: **33 tables / 1355 fields** (2026-08-30T18:38Z)
- Invalid formulas remaining after Asset Key fix: **0**
- Quarantined pending UI delete: **5**
- Empty / nearly-empty review queue: see `field-inventory/cleanup-queue.md`
...
**Airtable Meta API cannot delete fields** with the current token (PATCH works; DELETE → 404). Physical deletion requires Mike (or a token/plan that supports field delete) in the Airtable UI.
