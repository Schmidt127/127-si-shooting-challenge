...
| **Q8** | **Yes** — Production test before prod | **066 v3.1 already pasted in Production Airtable** — next step is audit/sandbox test, not re-paste unless GitHub version drift. Prod only after Production pass. |
...
**Question:** Automations **056**, **066**, and **101** may **skip** enrollments where `Active?` = false (Schmidt + Production test enrollments). Accept that for C-020 testing?
...
| **066** Production audit + sandbox test | Next Production session (paste already done) |
...
| Q3 | **012** deletion recorded — any automation still referencing 012 in interfaces? | OMNI scan Production + prod | **Approved 2026-07-05** — assume clean; focus forward |
...
| # | Question | Recommendation |
|---|----------|----------------|
| Q8 | **066 v3.1** — paste to Production only next session, prod only after Production audit pass? | Yes — per V2-015 |
| Q9 | Wave 2A complete in GitHub — mark V2-014 Wave 2A **done** after OMNI trigger confirm? | Yes |
| Q10 | **C-020** Test Intake — build on Production immediately after 066 Production pass, or parallel with OMNI table shell tonight? | Parallel OMNI schema + Cursor script **after** ChatGPT design lands |
...
| Q19 | 2026–27 gameplay numbers / XP tuning in Production | Wait for launch wave — not structural promote |
