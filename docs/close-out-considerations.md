...
| C-018 | 2026-07-03 | Weeks / calendar | **Intake open vs challenge run (date-driven Weeks)** | **Two calendars in config:** (1) **Intake open** — e.g. early bird partial week; setting start date = day app/Fillout accepts input; (2) **Challenge run** — official Week 1+ from challenge start date. All date ranges in **Weeks** table; **005 v4.1+** maps Activity Date fallback by Enrollment **Program Instance** then date range (homework-first unchanged). Optional per-row flags (`Intake Open?`, XP/leaderboard counts). | Design Weeks schema; 2026–27 rows scoped by Program Instance (**V2-013**); Fillout/web open gate on intake-open date. See [testing-and-intake-architecture.md](./testing-and-intake-architecture.md) § C-018 · [005-program-instance-week-scope.md](./deploy-checklists/005-program-instance-week-scope.md) | open |
