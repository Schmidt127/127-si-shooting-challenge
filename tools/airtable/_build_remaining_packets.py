...
OUTLETS = {"Billings Gazette", "Wibaux County Pioneer"}
...
# Athletes: Billings Gazette (incl. Bridger/Yellowstone) + Wibaux County Pioneer
plan_rows = []
seen_plan = set()
with open(BASE / "packet-plan-newspapers.csv", newline="", encoding="utf-8") as fh:
    for row in csv.DictReader(fh):
        if row.get("outlet_name") not in OUTLETS:
            continue
        key = (norm(row["athlete_included"]).lower(), norm(row["athlete_school"]).lower())
        if key in seen_plan:
            continue
        seen_plan.add(key)
...
for pr in plan_rows:
    ...
    if key in name_school_to_master:
        a = name_school_to_master[key]
        if a["enrollment_id"] in seen_athlete:
            continue
        seen_athlete.add(a["enrollment_id"])
        athletes.append(a)
    else:
...
HEADSHOT_CHECK = [
    ...
    "Primary outlet: Billings Gazette","Also serves: Carbon County News (Bridger), Wibaux County Pioneer",
