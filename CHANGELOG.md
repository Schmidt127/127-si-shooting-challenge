...
#### Changed
- **Automation 010 v10.5 replay-safety repair (2026-08-07)** — Validate a pre-existing single `Submission -> Weekly Athlete Summary` link against the current Submission `Enrollment + Week`, repair both Submission and Submission Base XP Event links to the canonical summary when exactly one safe replacement exists, and fail closed when no canonical repair target can be resolved. Added focused offline harness/tests: `tools/testing/tests/run_010_script.mjs` and `tools/testing/tests/test_010_offline.mjs`. Offline: `node --test tools/testing/tests/test_010_offline.mjs` (**PASS**). **Airtable paste unconfirmed; no controlled PROD live test performed.** [`010-submission-intake-create-xp-event.js`](./airtable/automations/shooting-challenge/010-submission-intake-create-xp-event.js) · [`AUTOMATION-010-XP-WRITER-RECONCILIATION.md`](./docs/prod-completion/2026-08-07/AUTOMATION-010-XP-WRITER-RECONCILIATION.md).
