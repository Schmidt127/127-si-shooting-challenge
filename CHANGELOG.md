...
#### Changed
- **SC-163 Goal Met Date timezone fix — 066 v4.1 (2026-09-05)** — Live v4.0 stamped Athlete1 as `2026-08-29` (UTC midnight → Denver) and dateTime display showed 8/28 6:00 PM. v4.1 preserves date-only Activity Date keys (no TZ convert of YYYY-MM-DD); chronology by date key → createdTime → id; schema must be date-only with time off. **Do not turn Production 066 ON until paste + retest.** No backfill in this fix. Checklist: [`docs/deploy-checklists/SC-163-goal-met-date.md`](./docs/deploy-checklists/SC-163-goal-met-date.md).
