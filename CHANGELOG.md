...
- **PROD: Program Homework Assignments MVP (2026-08-05)** — Additive junction table `Program Homework Assignments` (`tblhA3maf7xOa8EUS`) schedules reusable `FBC Curriculum - SYNC` by Program Instance + Week + Grade Band + Slot without editing library Week links. HC gains `Program Homework Assignment` link. Repo: **033 v3.2** (PHA-first + legacy fallback), **020 v3.1.0** (link PHA when resolvable). Perfect Week CASE-01 homework proof: assigned/satisfactory **2/2**. **033/020 PROD paste still required.** Separate from Perfect Week PR #81. [`program-homework-assignments-mvp.md`](./docs/deploy-checklists/program-homework-assignments-mvp.md).
