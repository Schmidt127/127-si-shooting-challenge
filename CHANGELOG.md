...
#### Added
- **FUT-009 — Safe post-feedback S3 video rename (2026-09-01)** — Copy-on-write rename worker at `lambda/upload-asset/upload_core/fut009_rename.py` with Option D destination keys (`shooting-challenge/` + FUT-007 VIDEO basename). CLI `tools/airtable/fut_009_video_rename.py` (dry-run default; `apply --confirm-rename`). Dual-prefix Storage Key validation aligns FUT-010 with grandfathered Gen B keys. Coach confirmation required (`Confirm S3 Video Rename` on VF and/or CLI flag). Old S3 objects retained. **No Production deploy, S3 apply, or schema paste.**
...
#### Added
- **FUT-009 promotion checklist (2026-09-01)** — [`docs/deploy-checklists/FUT-009-aws-storage-rename.md`](./docs/deploy-checklists/FUT-009-aws-storage-rename.md): coach confirmation, CopyObject workflow, failure recovery, PKG-004 audit fields, DEV proof steps.
