...
#### Changed
- **Homework late-credit policy (2026-09-03)** — **020 v3.9**, **065 v10.6**, **057 v2.3**: late satisfactory homework earns full `HOMEWORK_XP|{hcId}` credit; timing status `late` is recorded for reporting; Perfect Week homework counts require on-time Submission Date (PHA Due Date → Week End Date fallback). Contract tests: `tests/automation-contracts/065-homework-late-credit-policy.test.js`. Operator paste checklist (Mike-only after DEV): [`docs/deploy-checklists/homework-late-credit-policy-020-057-065.md`](./docs/deploy-checklists/homework-late-credit-policy-020-057-065.md). **Production paste pending — do not paste from agents.**
