...
#### Changed
- **Parent-email Live cutover prep (2026-09-02)** — **078A v1.5**: optional automation input `testMode` (default true) instead of hardcoded queue Test Mode. Production `Automations` table synced to v1.5. Preflight: zero real families; Hub `ready`/Resend; versions **071/072/073/074/076/079/117/118/119** MATCH. Disposable enrollments retargeted to `schmidt@fairfieldbasketballclub.com`. Fresh WELCOME handoff **Accepted** (queue `recySzWhjc26N2Yhl`, Hub Event `recyUkl2tRDRVmMpy`). Operator packet: [`docs/deploy-checklists/parent-email-live-cutover-2026-09-02.md`](./docs/deploy-checklists/parent-email-live-cutover-2026-09-02.md). Harness: `tools/testing/parent-email-live-cutover.mjs`. **Pending Mike:** paste 078A v1.5 into live Automations UI if table/UI drift; set queue-producer `testMode=false` and 118/119 Live schedule inputs per packet. Run `SEASON-SIM-2027-20260902T213135Z-athlete1` completed (writer complete after one idempotent resume). Verified cascade: 58 countable / 13,906 shots; Submission Base XP 58; streak XP + 17 Streak Occurrences; Zoom live 60 + recording 30; 6 WEEKLY handoffs Accepted/Hub sent to `schmidt@fairfieldbasketballclub.com` only; Perfect Week Eligible=0 (expected). Restored Production formulas (`Activity Date Is Future?` NOW()-only; `Submitted Same Day?` / `Perfect Week Grace Eligible?` rollbacks). Registry + XP/email extras cleaned. No 101 / SC-147 / 117 / 121 changes. Evidence: `tools/season_simulation/reports/evidence-final-SEASON-SIM-2027-20260902T213135Z-athlete1.json`.
