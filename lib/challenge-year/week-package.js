...
function generateWeekImportPackage(input = {}) {
  ...
  const importVerification = [
    "Row count matches generated plan (Early Bird / Week 0 + N + Post-Challenge)",
    "Each Start Date is Sunday; End Date is Saturday (America/Denver)",
    "Week Name labels exact: Early Bird (Week 0), Week 1..N, Post-Challenge",
  ...
  const warnings = [
    ...
    "Verify the production weekly-email schedule and Week End targeting before the first Sunday after import; this repository cannot attest its live state.",
  ];
