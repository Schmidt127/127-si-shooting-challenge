 ...
 * - When `lib/reliability-command-center` is present, map exclusively through
 *   the canonical RCC `buildIssue` contract (no silent alternate issue shape).
 * - Portable findings are returned only when the RCC module cannot be loaded
 *   at all (for example, older checkouts without PR #40).
 *
...
function buildSeasonReliabilityFindings(input = {}) {
  ...
  if (rcc) {
    if (typeof rcc.buildIssue !== "function") {
      throw new Error(
        "lib/reliability-command-center loaded but buildIssue is missing — refusing portable fallback that would create a second issue contract"
      );
    }
    return {
  ...
  return {
    ...
    findings: unique,
    note: "RCC module not present — portable findings only (no second issue contract invented).",
  };
}
