...
/**
 * This is deliberately an approval *request*, not an approval record. A value
 * in a local fixture proves only that someone proposed it; Mike's production
 * approval must still be recorded in the controlled activation evidence.
 */
function buildMikeDecisionSheet({ config = {}, levelPolicy = "undocumented" } = {}) {
  const value = (candidate) => (candidate == null || candidate === "" ? "**MISSING — blocks import and activation**" : `\`${candidate}\` (confirm)`);
  const normalizedPolicy = String(levelPolicy || "undocumented").trim().toLowerCase();
  const policy = VALID_LEVEL_POLICIES.has(normalizedPolicy)
    ? `\`${normalizedPolicy}\` (confirm)`
    : "**MISSING — choose `reset` or `carry`; blocks import and activation**";

  return [
    "# Mike season-launch decision sheet",
    "",
    "**Status:** Proposed local values only. This file does not approve, install, or activate anything.",
    "",
    "| Required decision | Proposed value | Mike approval/date |",
    "| --- | --- | --- |",
    `| Challenge year | ${value(config.challengeYearLabel)} | ☐ / __________ |`,
    `| Early Bird (Week 0) Sunday | ${value(config.weekZeroStart)} | ☐ / __________ |`,
    `| Regular-week count | ${value(config.regularWeekCount)} | ☐ / __________ |`,
    `| Level policy | ${policy} | ☐ / __________ |`,
    "",
    "## Gate",
    "",
    "Do not import Weeks, reopen intake, or activate the season until every row is confirmed and the dry run reports no FAIL findings. Retain this completed sheet with the dry-run packet and post-import reconciliation evidence.",
    "",
  ].join("\n");
}
...
function buildSeasonLaunchDryRun(input = {}) {
  ...
  result.decisionSheet = buildMikeDecisionSheet({ config, levelPolicy });
...
module.exports = { VALID_LEVEL_POLICIES, buildMikeDecisionSheet, findWeekConflicts, buildSeasonLaunchDryRun };
