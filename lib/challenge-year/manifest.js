...
const DEFAULT_WEB_ROUTES = [
  "/shoot leaderboard — current-year Enrollment filters",
  "/shoot levels + achievements — Enrollment year separation",
  "/shoot game-manual / catalogs — publish gate (field may still be Softr-named historically)",
  "Airtable views backing /shoot — must not mix historical seasons",
];
...
function generateRolloverManifest(input = {}) {
  ...
  const manifest = {
    ...
    webRoutesAndFiltersToInspect:
      input.webRoutesAndFiltersToInspect ||
      input.softrPagesAndFiltersToInspect || // Historical Reference Only key — ignored for Softr
      DEFAULT_WEB_ROUTES,
    softrPagesAndFiltersToInspect: {
      status: "Obsolete",
      note: "Softr is Not Used for Shooting Challenge. Use webRoutesAndFiltersToInspect.",
      historical: input.softrPagesAndFiltersToInspect || null,
    },
...
function renderManifestMarkdown(manifest, weekPlan) {
  ...
  const sections = [
    ...
    ["Website (/shoot) routes and filters", manifest.webRoutesAndFiltersToInspect],
    ["Softr (Obsolete / Not Used)", ["Historical Reference Only — not a launch dependency"]],
