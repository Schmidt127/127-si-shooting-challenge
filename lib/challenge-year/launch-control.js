...
function launchPreflight(input = {}) {
  ...
  const levelPolicyCheck = ["reset", "carry"].includes(levelPolicy)
    ? { severity: "PASS", code: "level_policy_explicit", message: `Level policy selected: ${levelPolicy}.` }
    : { severity: "FAIL", code: "level_policy_decision_required", message: "Level policy must be explicitly reset or carry before a season can be launch-ready.", requiredAction: "Mike must choose levelPolicy=reset or levelPolicy=carry; do not assume carry-over." };
