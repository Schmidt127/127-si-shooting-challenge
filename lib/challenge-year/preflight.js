...
function runRolloverPreflight(input = {}) {
  ...
  // Softr checklist keys are Obsolete — map legacy fixture flags onto web checks.
  if (ops.webNotHardCoded == null && ops.softrNotHardCoded != null) {
    ops = { ...ops, webNotHardCoded: ops.softrNotHardCoded };
  }
  ...
  const requiredOps = [
    ...
    [
      "currentYearViewsDocumented",
      "Current-year views documented",
      "Document Airtable + /shoot current-year view/query filters (Softr is Obsolete / Not Used).",
    ],
    ["makeNotHardCoded", "Make scenarios not hard-coded to old year", "Inspect Make for hard-coded season strings."],
    [
      "webNotHardCoded",
      "Website (/shoot) queries not hard-coded to old year",
      "Inspect Next.js Airtable queries/views for hard-coded season strings.",
    ],
  ];
