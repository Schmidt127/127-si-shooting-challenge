...
const { toDateKey, isDateKeyInWeekRange, countDistinctQualifyingDays } =
  require("./weekly-summary-email-content");
const {
  countDistinctPerfectWeekQualifyingDays,
  resolvePerfectWeekSubmissionGraceHours,
} = require("./perfect-week-submission-timing");
 ...
 */
/**
 * Build shooting-day vs Perfect Week qualifying-day metrics from submission snapshots.
 * @param {Array<object>} submissions
 * @param {{ weekStartKey?: string, weekEndKey?: string, graceHours?: number|null, nowMs?: number }} bounds
 */
function buildShootingAndPerfectWeekDayMetrics(submissions, bounds = {}) {
  const { weekStartKey = "", weekEndKey = "" } = bounds;
  const graceHours = resolvePerfectWeekSubmissionGraceHours(bounds.graceHours);
  const shootingSubs = (submissions || []).filter((row) => {
    if (!row?.countThisSubmission && row?.countable !== true) return false;
    if (!row?.activityDateKey) return false;
    return isDateKeyInWeekRange(row.activityDateKey, weekStartKey, weekEndKey);
  });
  const shootingDaysLogged = countDistinctQualifyingDays(
    shootingSubs.map((row) => ({ activityDateKey: row.activityDateKey }))
  );
  const perfectWeekDaysLogged = countDistinctPerfectWeekQualifyingDays(submissions, {
    weekStartKey,
    weekEndKey,
    graceHours,
    nowMs: bounds.nowMs,
...
function buildPerfectWeekEmailCriteria(input = {}) {
  ...
  return {
    ...
    perfectWeekQualifyingDaysDisplay: formatDaysLoggedAgainstCriteria(
      daysLogged,
      requiredShootingDays
    ),
    videoCount,
...
module.exports = {
  ...
  buildShootingAndPerfectWeekDayMetrics,
