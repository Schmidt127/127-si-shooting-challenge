...
const {
  ...
  isExternalVerificationComplete,
  passesOperatorS3Gate,
...
test("4b. Gen B Storage Key passes dual-prefix FUT-010 verification", () => {
  const genBKey =
    "Schmidt_Xavier/Shooting_Challenge_2026-2027/2026-08-17/" +
    "20260817T172732Z_VIDEO_recAqoUbBKfDNtTLt_clip.mp4";
  const decision = decideIntakeAttachmentCleanup({
    recordId: "recGENBVIDEO",
    fields: baseUploadedFields({
      "Upload Destination": "Video Feedback",
      "Asset Purpose": "Video For Feedback",
      "Storage Key": genBKey,
      "Reviewer File URL": LAMBDA_VIEWER_URL,
      "Airtable Attachment": [{ id: "attGENB", filename: "clip.mp4" }],
    }),
    externalChecks: externalOk(),
    dryRun: true,
  });
  assert.strictEqual(decision.verification.aws.checks.storageKeyFormatValid, true);
});
...
test("12. missing external verification fails closed", () => {
  const decision = decideIntakeAttachmentCleanup({
    recordId: "recNOEXT001",
    fields: baseUploadedFields(),
    externalChecks: {},
    dryRun: false,
  });
  assert.strictEqual(decision.action, "skipped_verification_failed");
  assert.match(decision.failureReason, /External verification incomplete/);
});

test("13. s3ObjectExists must be explicitly true", () => {
  const decision = decideIntakeAttachmentCleanup({
    recordId: "recS3FALSE",
    fields: baseUploadedFields(),
    externalChecks: { s3ObjectExists: false, canonicalUrlReachable: true },
  ...
  assert.strictEqual(decision.action, "skipped_verification_failed");
  assert.strictEqual(isExternalVerificationComplete({ s3ObjectExists: false, canonicalUrlReachable: true }), false);
});

test("14. canonicalUrlReachable must be explicitly true", () => {
  const partial = decideIntakeAttachmentCleanup({
    recordId: "recCANONPART",
    fields: baseUploadedFields(),
    externalChecks: { s3ObjectExists: true },
    dryRun: false,
  });
  assert.strictEqual(partial.action, "skipped_verification_failed");
  assert.match(partial.failureReason, /External verification incomplete|Canonical File URL probe failed/);

  const falseProbe = decideIntakeAttachmentCleanup({
    recordId: "recCANONFALSE",
    fields: baseUploadedFields(),
    externalChecks: { s3ObjectExists: true, canonicalUrlReachable: false },
    dryRun: false,
  });
  assert.strictEqual(falseProbe.action, "skipped_verification_failed");

  const aws = evaluateAwsVerification(baseUploadedFields(), "homework", { s3ObjectExists: true });
  assert.strictEqual(aws.checks.canonicalUrlReachable, false);
});

test("15. extension VERIFY_S3_OBJECT_EXISTS=false refuses deletion", () => {
  assert.strictEqual(passesOperatorS3Gate(false), false);
  assert.strictEqual(passesOperatorS3Gate(undefined), false);
  assert.strictEqual(passesOperatorS3Gate(true), true);
});
