...
const { isValidStorageKeyFormat } = require("../s3-storage/storage-key-format.js");
const {
...
function evaluateAwsVerification(fields, category, external = {}) {
  ...
  const checks = {
    storageKeyFormatValid: isValidStorageKeyFormat(storageKey),
    canonicalUrlHttps: /^https:\/\//i.test(canonicalUrl),
    s3ObjectExists: external.s3ObjectExists === true,
    canonicalUrlReachable: external.canonicalUrlReachable === true,
    externalVerificationComplete: isExternalVerificationComplete(external),
  };
  ...
  if (external.s3ObjectExists === false) {
    reason = "S3 object not found at Storage Key.";
  } else if (external.canonicalUrlReachable === false) {
    reason = "Canonical File URL probe failed.";
  } else if (!checks.externalVerificationComplete) {
    reason = "External verification incomplete — S3 HeadObject and canonical URL probe required.";
  } else if (!checks.storageKeyFormatValid) {
...
/**
 * Fail-closed gate for operator-provided S3 verification (extension VERIFY_S3_OBJECT_EXISTS).
 * @param {unknown} verifyFlag
 * @returns {boolean}
 */
function passesOperatorS3Gate(verifyFlag) {
  return verifyFlag === true;
}

/**
 * Fail-closed gate for programmatic external verification inputs.
 * @param {ExternalVerificationChecks} external
 * @returns {boolean}
 */
function isExternalVerificationComplete(external = {}) {
  return external.s3ObjectExists === true && external.canonicalUrlReachable === true;
}
